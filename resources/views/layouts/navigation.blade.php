<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
            ...
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('anuncio.index') }}">
                        <x-application-logo class="block h-9 w-auto fill-current text-orange-400 dark:text-gray-200" />
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                @if(Auth::check())
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-orange-400 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                                <div>{{ Auth::user()->nome->nome }}</div>
                                <div class="ml-1">
                        ...
                        <x-slot name="content">
                            ...
                            <x-dropdown-link :href="route('agendado.index')">
                                {{ __('Visualizar Reservas') }}
                            </x-dropdown-link>

                            @if(Auth::check() && Auth::user()->tipousu != 'Cliente')
                                <x-dropdown-link :href="route('meus-anuncios')">
                                    {{ __('Meus Anúncios') }}
                                </x-dropdown-link>
                            @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            @if(Auth::check())
                <div class="px-4">
                    <div class="font-medium text-base text-gray-800 dark:text-gray-200">{{ Auth::user()->nome->nome }}</div>
                ...
                <div class="mt-3 space-y-1">
                    ...
                    <x-responsive-nav-link :href="route('agendado.index')">
                        {{ __('Visualizar Reservas') }}
                    </x-responsive-nav-link>

                    @if(Auth::check() && Auth::user()->tipousu != 'Cliente')
                        <x-responsive-nav-link :href="route('meus-anuncios')">
                            {{ __('Meus Anúncios') }}
                        </x-responsive-nav-link>
                    @endif
            ...
            @else
                <div class="px-4">
                    <a href="{{ route('login') }}" class="block bg-blue-500 text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-blue-600 mb-2">Login</a>
                    <a href="{{ route('register') }}" class="block bg-green-500 text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-green-600">Registrar</a>
                </div>
            @endif
        </div>
    </div>
